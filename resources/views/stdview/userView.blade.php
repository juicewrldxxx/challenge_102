            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item_user->phoneNumber }}</td>
                        <td>
                            <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#exampleModal{{ $item_user->id }}">Message</button>
                            <a href="{{ route('user.show',['id' => $item_user->id]) }}">
                                <button type="button" class="btn btn-success" data-toggle="modal">Detail</button>
                            </a>
                            @if(Auth::user()->role)
                            <a href="{{ route('user.edit',['user' => $item_user->id]) }}">
                                <button type="button" class="btn btn-primary">Update</button>
                            </a>
                            <a href="{{ route('user.destroy',['user' => $item_user->id]) }}">
                                <button type="button" class="btn btn-danger">Delete</button>
                            </a>
                            @endif
                        </td>
                    </tr>
                    <div class="modal fade " id="exampleModal{{ $item_user->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">Room chat with {{ $item_user->username }}</h5>
                                ...
                                <div class="modal-body">
                                    @php
                                        $userRooms = Auth::user()->room;
                                        $roomChat = "";

                                        if($userRooms){
                                            foreach ($userRooms as $key => $room){
                                                $userInRoom = DB::table('user_room')->where('room_id',$room->id)->where('user_id',$item_user->id)->get();
                                                if(count($userInRoom) == 0){
                                                }else{
                                                    $roomChat = $room;
                                                }
                                            }
                                        ...
                                        if($roomChat !== "") {
                                            $messages = \App\Models\message::where('room_id',$roomChat->id)->get();

                                        }

                                        if($roomChat){
                                            $s = "false";
                                            $results = $roomChat->user;
                                            foreach ($results as $key => $result){
                                                    if($result->id == $item_user->id){
                                                        $s = "true";
                                                    }
                                                }
                                        }else{
                                    ...
                                    @if(count($messages) == 0)
                                        <h1>Let start talk with {{ $item_user->username }}</h1>
                                    @else

                                    @foreach ($messages as $key => $message_item)
                                    @if($message_item->user_id == Auth::user()->id)
                                ...
                                <form class="form-inline" style="display:flex;" action="{{ route('message.send',['userSend_id' => Auth::user()->id,'userGet_id' => $item_user->id , 'room_id' => $roomChat ? $roomChat->id : '']) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
