...
<div id="main-content" class="container allContent-section py-4">
    <div class="d-flex flex-row w-100">
        <img src="https://images.unsplash.com/photo-1621359748534-8a83aba26934?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=387&q=80" alt="..." class="img-thumbnail w-30 p-3">
        <div class="user__infor">
            <h1>{{ $user->username }}</h1>
            <p><i class="fa-solid fa-phone-volume"></i> PhoneNumber : {{ $user->phoneNumber }}</p>
            <div class="card" id="card">
                ...
                <div class="card-body">
                    ...
                    $userInRoom = DB::table('user_room')->where('room_id',$room->id)->where('user_id',$user->id)->get();
                    ...
                    @if(count($messages) == 0)
                    <h1>Let start talk with {{ $user->username }}</h1>
                    @else
                    ...
                    @endif
                    <form class="form-inline w-100 form-send-message" style="display:flex; margin-top:20px;" action="{{ route('message.send',['userSend_id' => Auth::user()->id,'userGet_id' => $user->id , 'room_id' => $roomChat ? $roomChat->id : '']) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="text" class="form-control form-input" id="InputName" placeholder="Enter message" name="text">
